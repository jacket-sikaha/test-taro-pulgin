...
export class BaseTemplate {
  ...
  protected modifyCompProps?: (
    compName: string,
    target: Record<string, string>
  ) => Record<string, string>;
  protected modifyLoopBody?: (child: string, nodeName: string) => string;
  protected modifyLoopContainer?: (
    children: string,
    nodeName: string
  ) => string;
  protected modifyTemplateResult?: (
    res: string,
    nodeName: string,
    level: number,
    children: string
  ) => string;
  protected modifyThirdPartyLoopBody?: (
    child: string,
    nodeName: string
  ) => string;
        ...
        for (let prop in component) {
          if (hasOwn(component, prop)) {
            ...
            if (prop.startsWith("bind") || propValue === "eh") {
              ...
              const propAlias =
                componentAlias[propInCamelCase] || propInCamelCase;
  ...
  protected buildBaseTemplate() {
    console.log("buildBaseTemplate------------------------------------");
    const Adapter = this.Adapter;
    const data =
      !this.isSupportRecursive && this.supportXS
        ? `${this.dataKeymap(
            `i:item,c:1,l:xs.f('',item.${Shortcuts.NodeName})`
          )}`
...
`;
  ...
  protected buildThirdPartyAttr(
    ...
    return Array.from(attrs).reduce((str, attr) => {
      ...
      if (
        isBooleanStringLiteral(patchValue) ||
        isNumber(patchValue) ||
        isString(patchValue)
      ) {
        const propValue = this.supportXS
          ? `xs.b(i.${toCamelCase(attr)},${patchValue})`
          : `i.${toCamelCase(attr)}===undefined?${patchValue}:i.${toCamelCase(
              attr
            )}`;
  ...
  private getChildrenTemplate(level: number) {
    ...
    const isLastRecursiveComp =
      !isSupportRecursive && level + 1 === this.baseLevel;
    ...
    if (isLastRecursiveComp) {
      ...
      const data = isUseXs
        ? `${this.dataKeymap(
            `i:item,c:c+1,l:xs.f(l,item.${Shortcuts.NodeName})`
          )}`
        : this.isSupportRecursive
        ? `${this.dataKeymap("i:item")}`
...
<template name="tmpl_${level}_${nodeAlias}">
  <template is="{{${templateName}}}" data="{{${
      this.isSupportRecursive
        ? this.dataKeymap("i:i")
        : this.dataKeymap("i:i,c:c")
    }}}" />
</template>
...
<template name="tmpl_${level}_${nodeAlias}_focus">
  <${nodeName} ${this.buildAttribute(
      comp.attributes,
      nodeName
    )} id="{{i.uid||i.sid}}" data-sid="{{i.sid}}">${children}</${nodeName}>
</template>
...
<template name="tmpl_${level}_${compName}">
  ...
  protected buildContainerTemplate(level: number) {
    const tmpl = `<block ${this.Adapter.if}="{{i.nn === '${
      this.componentsAlias["#text"]._num
    }'}}">
    <template is="tmpl_0_${
      this.componentsAlias["#text"]._num
...
<template is="{{'tmpl_0_' + i.nn}}" data="{{${data}}}" />`;
  ...
  public mergeThirdPartyComponents(
...
export class RecursiveTemplate extends BaseTemplate {
  ...
  public buildTemplate = (componentConfig: ComponentConfig) => {
    ...
    template = components.reduce((current, nodeName) => {
      ...
      return (
        current +
        this.buildComponentTemplate(
          { nodeName, nodeAlias, attributes },
          ZERO_FLOOR
        )
      );
    }, template);
...
export class UnRecursiveTemplate extends BaseTemplate {
  ...
  protected buildFloor(level: number, components: string[], restart = false) {
    ...
    let template = components.reduce((current, nodeName) => {
      ...
      return (
        current +
        this.buildComponentTemplate({ nodeName, nodeAlias, attributes }, level)
      );
    }, "");
  ...
  protected buildXSTmplName() {
    ...
    const listA = Array.from(isLoopCompsSet).map(
      (item) => componentsAlias[item]?._num || item
    );
    const listB = hasMaxComps.map(
      (item) => componentsAlias[item]?._num || item
    );
